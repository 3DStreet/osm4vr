...
AFRAME.registerComponent('osm-tiles', {
  ...
  lat2tileWidth_m: function (lat, zoom) {
    ...
    const nTiles = 2 ** zoom;
    const circumference_m = EQUATOR_M * Math.cos(lat * Math.PI / 180);
  ...
  loadTile: function (x, y) {
    const url = this.data.url + `${this.data.zoom}/${x}/${y}.png`;
    const x_m = (x - this.tileBase[0] + 0.5) * this.tileSize_m;
    const y_m = (y - this.tileBase[1] + 0.5) * this.tileSize_m;
    const tile = this.createTile(x_m, -y_m, url);
